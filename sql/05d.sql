...
WHERE film_id IN (
    SELECT film_id FROM film_actor WHERE actor_id IN (
        SELECT actor_id FROM film_actor JOIN film USING (film_id) WHERE title = 'AMERICAN CIRCUS'
    )
    INTERSECT
    SELECT film_id FROM film_actor WHERE actor_id IN (
        SELECT actor_id FROM film_actor JOIN film USING (film_id) WHERE title = 'ACADEMY DINOSAUR'
    )
    INTERSECT
    SELECT film_id FROM film_actor WHERE actor_id IN (
        SELECT actor_id FROM film_actor JOIN film USING (film_id) WHERE title = 'AGENT TRUMAN'
    )
);
